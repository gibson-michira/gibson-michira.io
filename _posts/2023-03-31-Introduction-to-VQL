...
Before we move any further, let us actually run some queries, so we can get familiar with VQL. For this scenario, you are provided with a windows VM and the Velociraptor binary. Yes, the beauty of velociraptor is, we don’t really need to have a fleet of endpoints and a dedicated server when trying this stuff out. We can simply run the executable on the VM (endpoint) in question, and instantly have an instance of Velociraptor to work with. Neat right?
